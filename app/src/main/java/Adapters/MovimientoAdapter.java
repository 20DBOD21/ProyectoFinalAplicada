...
import com.example.proyectobilleteradigital.R;
...
public class MovimientoAdapter extends RecyclerView.Adapter<MovimientoAdapter.MovimientoViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MovimientoAdapter.MovimientoViewHolder holder, int position) {
        ...
        Categoria categoria = categoriaMap.get(movimiento.getIdCategoria());

        if (categoria != null) {
            holder.tv_cat_dash.setText(categoria.getNombre());
            int recursoId = context.getResources().getIdentifier(categoria.getIcono(), "drawable", context.getPackageName());
            ...
            holder.tv_cat_dash.setText("Categoria Desconocida");
            holder.iv_icon_category.setImageResource(R.drawable.logo);
        }
        ...
        if (movimiento.isEsGasto()) {
            ...
            holder.tv_monto_mov_dash.setTextColor(ContextCompat.getColor(context, R.color.colorToolbar));
        }
    }
